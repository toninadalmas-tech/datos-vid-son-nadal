...
def calcular_pluja_10d(df: pd.DataFrame) -> pd.Series:
    """Precipitació acumulada en finestra mòbil de 10 dies."""
    p = pd.Series(pd.to_numeric(df["precipitacio_mm"], errors="coerce").fillna(0).values, index=df["ts"])
    res = p.rolling('10D', min_periods=1).sum().round(2)
...
def calcular_t_mitjana_10d(df: pd.DataFrame) -> pd.Series:
    ...
    t = pd.Series(pd.to_numeric(df["temperatura_c"], errors="coerce").values, index=df["ts"])
    res = t.rolling('10D', min_periods=1).mean().round(2)
    return pd.Series(res.values, index=df.index)
